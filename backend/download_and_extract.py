...
def try_extract(text, regex_pattern, shape_pattern, val_type):
    ...
    if not raw:
        return None
    if shape_pattern:
        if not re.search(shape_pattern, raw, re.IGNORECASE):
            return None
...
def resolve_city_for_gem(location_text):
    ...
    for city in sorted_cities:
        ...
        if city_lower == 'korea':
            if re.search(r'\b(south\s+korea|korean|made\s+in\s+korea|origin\s*:?\s*korea|import\w*\s+from\s+korea)\b', text, re.IGNORECASE):
                continue
...
def process_tender_extraction(tender, tenders_col):
    ...
    print(f"[*] Extracting details from local PDF: {full_pdf_path}")
    ...
    if not updated_state or updated_state == 'Unspecified':
        ...
        for state in sorted(states_list, key=len, reverse=True):
            escaped_state = re.escape(state)
            if re.search(r'\b' + escaped_state + r'\b', haystack, re.IGNORECASE):
                updated_state = state
                print(f"[+] Resolved state from PDF: {updated_state}")
                break
    ...
    if not updated_city or updated_city == 'Unspecified':
        ...
        if resolved and resolved != 'Unspecified':
            ...
            print(f"[+] Resolved city from PDF: {updated_city}")
        ...
        if os.path.exists(old_full_path):
            ...
            if os.path.normpath(old_full_path) != os.path.normpath(new_full_path):
                try:
                    ...
                    print(f"[+] Moved PDF file to state folder: {pdf_path}")
                except Exception as e:
                    print(f"[-] Failed to rename/move PDF: {e}")
    ...
    print(f"[+] Database updated for {tender['bidNumber']} in {elapsed:.2f}s: bidValue={result['bidValue']}, emdAmount={result['emdAmount']}")
    return True
...
def download_gem_pdf(tender, documents_dir):
    ...
    if not gem_id:
        print(f"[-] GeM tender {tender['bidNumber']} has no gemId — cannot download PDF")
        return None
    ...
    try:
        ...
        if response.status_code == 200 and 'pdf' in response.headers.get('content-type', '').lower():
            ...
            print(f"[+] Downloaded GeM PDF: {file_path}")
            return file_path
        else:
            print(f"[-] GeM PDF download failed for {tender['bidNumber']}: HTTP {response.status_code}")
    except Exception as e:
        print(f"[-] Error downloading GeM PDF for {tender['bidNumber']}: {e}")
...
def process_single_tender_download(tender, documents_dir, tenders_col):
    ...
    if os.path.exists(full_local_path) and os.path.getsize(full_local_path) > 1000:
        return True # Skipped downloading since it's already here
        
    # 2. File does not exist locally. Download it.
    local_path = None
    if source == 'GEM':
        local_path = download_gem_pdf(tender, documents_dir)
...
def scrape_gem_listings_for_state(session, csrf, state_name, tenders_col):
    import math
    print(f"\n[*] Syncing listings for state: {state_name}...")
    num_found, docs = fetch_page_state_full(session, csrf, state_name, 1)
    if num_found == 0:
        print(f"[-] No active bids found for {state_name}.")
        return 0
    ...
    print(f"[+] Scraped {len(docs)} raw listings for {state_name}.")
...
def main():
    ...
    for idx, state_name in enumerate(states_to_process):
        ...
        try:
            ...
            print(f"[-] Scraper failed for {state_title}: {e}")
        ...
        print(f"[*] Checked local disk for {state_title}: {already_downloaded_count}/{len(all_tenders)} present.")
        ...
        if total_pending > 0:
            ...
            print(f"[+] Download complete for {state_title}: {downloaded_count}/{total_pending} downloaded successfully in {download_time:.2f}s.")
        else:
            print(f"[+] All PDFs for {state_title} are already present.")
