...
// source: 'GEM' | 'CSPGCL' | undefined (undefined = show all three badges)
export default function TendersStatsBar({ source } = {}) {
  ...
  // Titles / descriptions per source variant
  const heading = source === 'GEM' ? 'GeM Tenders'
    : source === 'CSPGCL' ? 'CSPGCL Tenders'
    : 'All Tenders';

  const subtext = source === 'GEM'
    ...
    <div className="bg-white border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-6">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 font-['Outfit']">{heading}</h1>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {/* All-sources view: show Open + GEM + CSPGCL */}
            {!source && (
              <>
            ...
            {source === 'GEM' && (
              <div className="flex items-center gap-2 bg-violet-50 border border-violet-100 rounded-xl px-3 py-2">
                {stats
                  ? <span className="text-xl font-extrabold text-violet-700 font-['Outfit'] leading-none">{stats.bySource?.GEM || 0}</span>
                  : <div className="h-6 w-10 bg-violet-100 rounded animate-pulse" />
                }
                <span className="text-xs text-violet-600 font-medium leading-tight">GeM Tenders<br />Open</span>
              </div>
            )}
            {/* CSPGCL-only view */}
            {source === 'CSPGCL' && (
              <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2">
                ...
                <span className="text-xs text-amber-600 font-medium leading-tight">CSPGCL Tenders<br />Open</span>
              </div>
            )}
          </div>
        </div>
